<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://images.pling.com/cache/85x85-2/img/00/00/84/48/12/2345743/andryemu.png">
    ![dgc](https://github.com/user-attachments/assets/133e2952-a470-490a-b554-9336c36c1049)

  </picture>
  
  <h1>Dual Gaming Centre</h1>
  
  <p>
    <strong>One Portal. All Eras. No Limits.</strong><br>
    Play any game online without registration!
  </p>

  <p>
    <a href="https://github.com/Kolyadual/andryemu/releases">
      <img src="https://img.shields.io/github/v/release/Kolyadual/dualgamingcentre?style=flat&color=6366f1&logo=github" alt="Release">
    </a>
    <a href="https://github.com/Kolyadual/dualgamingcentre/blob/main/LICENSE">
      <img src="https://img.shields.io/badge/license-GPLv3-3b82f6.svg?style=flat" alt="License">
    </a>
    <a href="https://github.com/Kolyadual/dualgamingcentre/stargazers">
      <img src="https://img.shields.io/github/stars/Kolyadual/dualgamingcentre?style=flat&color=fbbf24" alt="Stars">
    </a>
    <a href="https://github.com/Kolyadual/dualgamingcentre/issues">
      <img src="https://img.shields.io/github/issues/Kolyadual/dualgamingcentre?style=flat&color=10b981" alt="Issues">
    </a>
  </p>

  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://via.placeholder.com/800x400/0f172a/94a3b8?text=Running+Telegram+on+Asus+EEE+PC+1005HA">
    <img width="639" height="319" alt="Снимок экрана_20260223_213142" src="https://github.com/user-attachments/assets/e99abe6f-efdd-435f-a653-f124c4556775" />

  </picture>

[**🇺🇸 English**](#english-version) | [**🇷🇺 Русский**](#russian-version)

---

<div id="english-version"></div>

## 🌐 English Version

### Play any game online without registration!
From legendary **DOOM** and **Minecraft** to mobile hits like **Jetpack Joyride**. If you love a game, you can easily download it for **Runget**, **Windows**, or **Android**!

### 🕹️ Features
*   **Ultimate Alternative:** A 100% FREE alternative to VK Play or Yandex Games. **No ads, no subscriptions.** Made with soul for gamers worldwide!
*   **Virtual Emulator:** Built-in powerful engine. Search for "Virtual Emulator" on the site to run your own ROMs: **PSP, PS1, Sega, NES, SNES, GBA, N64**, and more.
*   **Events:** Participate in free tournaments and competitions.
*   **Daily Updates:** New games added every single day.
*   **Community Powered:** Join our [Telegram](https://t.me/DGC_off) to request any game you want to see!

### ⚙️ System Requirements
*   **Speed:** Play smoothly even with **32 Mbps** internet.
*   **Performance:** Stable **35-60 FPS** for all titles.
*   **Controls:** Full support for **Gamepads**, Joysticks, Keyboards, and Touchscreens.

</div>

---

<div id="russian-version"></div>

## 🇷🇺 Русская версия

### Играй в любые игры без регистрации онлайн!
От легендарных **DOOM** и **Minecraft** до мобильных хитов вроде **Jetpack Joyride**. Если игра понравилась — её можно легко скачать для **Runget**, **Windows** или **Android**!

### 🔥 Фичи, ради которых стоит попробовать
1.  **Настоящая свобода:** Полностью БЕСПЛАТНАЯ альтернатива VK Play и Яндекс Играм. **Без рекламы и подписок.** Сделано с душой в России — для людей по всей земле!
2.  **Virtual Emulator:** Наш встроенный эмулятор (найди его через поиск на сайте) позволяет запускать твои локальные ROM-файлы: **PSP, Sega Mega Drive, NES, SNES, GBA, N64, PS1** и многих других консолей.
3.  **Ивенты:** Участвуй в бесплатных соревнованиях и событиях.
4.  **Ежедневный рост:** Библиотека игр пополняется каждый день.
5.  **Твой выбор:** Подпишись на [Telegram-канал](https://t.me) и пиши разработчикам напрямую, какую игру добавить следующей!

### ⚙️ Требования и возможности
*   **Интернет:** Комфортная игра даже при **32 Мбит/с**.
*   **Фреймрейт:** Стабильные **35-60 FPS** во всех играх.
*   **Управление:** Полная поддержка **геймпадов**, джойстиков, клавиатур и сенсора.

---

## 🔗 Quick Links / Быстрые ссылки
*   🚀 **[Open Dual Gaming Centre](https://kolyadual.github.io)**
*   📢 **[Official Telegram Channel](https://t.me)**
*   🧪 **[Tester's Channel](https://t.me)**

---

## 🛡️ Disclaimer / Отказ от ответственности
*ALL GAMES available in Dual Gaming Centre are NOT developed by Kolyadual. All rights belong to their respective developers and owners. Please play responsibly!*

## 🌟 Other Projects / Другие проекты
*   💻 **[Runget OS](https://github.com)**
*   🤖 **[Andryemu Android Emulator](https://github.com)**

<div align="center">
  <p>Made with ❤️ by <b>Kolyadual</b></p>
</div>
